...
A feature `todos` é um **exemplo read-only** (GET + filtro via `?filter=`), não
um CRUD completo. Densidade da lista (`listDensityAtom`) demonstra Atom para
estado de UI fora da URL.
